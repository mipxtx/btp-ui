var DashboardsHelpers = {
	makegraph_srv: function (params, items, scale, slice, caption) {
		...
		return '<img width="530" height="250" src="graph.php?params=' + u_params + p_caption + '&items=' + u_items + '&scale=' + scale + '&slice=' + slice + '&host=' + window.btp.getHost() + '"> ';
	},
...
var Dashboards = {
	...
	php_top_front_cnt: function (node) {
		...
		btp.query('get_list_advanced', _.extend(params, {script: '?', sort_by: 'count', limit: 10, scale: App.scale}), function (res) {
	...
	php_top_front_total: function (node) {
		var params = {service: 'SCRIPT_wwwnew', op: 'all'};
		btp.query('get_list_advanced', _.extend(params, {script: '?', sort_by: 'total', limit: 10, scale: App.scale}), function (res) {
			var html = DashboardsHelpers.makegraphlist(res, params, 'script', "#script/@@/SCRIPT_wwwnew/");
			node.html(html);
		});
	},
	...
	php_top_front_time: function (node) {
		var params = {service: 'SCRIPT_wwwnew', op: 'all'};
		btp.query('get_list_advanced', _.extend(params, {script: '?', sort_by: 'perc80', limit: 10, scale: App.scale}), function (res) {
			var html = DashboardsHelpers.makegraphlist(res, params, 'script', "#script/@@/SCRIPT_wwwnew/");
			node.html(html);
		});
	},
	...
	top_service: function (node) {
		var params = {service: '?'};
		btp.query('get_list_advanced', _.extend(params, {limit: 30, sort_by: 'count', scale: App.scale}), function (res) {
